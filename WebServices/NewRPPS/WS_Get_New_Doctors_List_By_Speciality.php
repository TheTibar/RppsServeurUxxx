...
if (! empty($region_token) && ! empty($user_token) && ! empty($process_id))
{
    ...
    $result = $Doctor->getArrivingDoctors($process_id, $region_id);
    switch($result) {
        case 0:
            response(200, 'new_doctor_speciality_count', $Doctor->__get('create_doctors_array'));
            break;
        case 1:
            response(200, 'no_new_doctor', NULL);
            break;
        case -4:
            response(200, 'error_retrieving_arriving_doctors', NULL);
            break;
    }
}
else
