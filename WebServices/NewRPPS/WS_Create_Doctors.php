...
if (! empty($region_token) && ! empty($user_token))
{
    ...
    $result = $Process->newProcess($region_id, "ADD DOCTORS", 0);
    switch($result) {
        case 0:
    ...
    $result = $Doctor->createCreateDoctors($process_id, $region_id);
    switch($result) {
        case 0:
            $data['nb_create'] = $Doctor->__get('nb_create');
            $resp = 0;
            break;
        case 1:
            $resp = 1;
        ...
        case -1:
            ...
            break;
        case -2:
            response(200, "create_transaction_error", NULL);
            break;
        case -3:
            response(200, "create_create_data_error", NULL);
            break;
        case -4:
            response(200, "count_create_data_error", NULL);
            break;
    }
    ...
    switch($result) {
        case 0:
            if ($resp == 0)
            {
                response(200, "create_count", $data);
            }
            else
            {
                response(200, "no_create", NULL);
            }
            break;
        case -1:
