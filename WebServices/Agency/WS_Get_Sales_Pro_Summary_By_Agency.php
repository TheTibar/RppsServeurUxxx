...
if (! empty($agency_token) && ! empty($user_token))
{
    ...
    $result = $Agency->getSalesProSummaryByAgency($agency_id);
    
    switch($result) {
        case 0:
            $data['agency_sales_pro'] = $Agency->__get('sales_pro_array');
            $data['sales_pro_region'] = $Agency->__get('sales_pro_region');
    ...
    switch($result) {
        case 0:
            $data['agency_region'] = $Agency->__get('region_array');
            response(200, "agency_sales_pro", $data);
            break;
        case 1:
            response(200, "no_agency_region", NULL);
            break;
        case -3:
